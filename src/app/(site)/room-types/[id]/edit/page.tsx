...
export default function EditRoomTypePage({ params }: { params: { id: string } }) {
  ...
  const fetchRoomType = async () => {
    try {
      setLoading(true);
      const data = await httpClient.get<RoomType>(`/RoomTypes/${params.id}`);
      setRoomType(data);
      setFormData({
        ...
        active: data.active,
        occupancyPrices: data.occupancyPrices || [],
      });
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Erro ao carregar tipo de quarto');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        occupancyPrices: formData.occupancyPrices.length > 0 ? formData.occupancyPrices.map(p => ({
          occupancy: p.occupancy,
          pricePerNight: p.pricePerNight
        })) : undefined,
      };

      await httpClient.put(`/RoomTypes/${params.id}`, payload);
      toast.success('Tipo de quarto atualizado com sucesso!');
      router.push(`/room-types?hotelId=${formData.hotelId}`);
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Erro ao atualizar tipo de quarto');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="container mx-auto max-w-4xl px-4">
        <div className="mb-8">
          ...
          <p className="mt-2 text-body-color dark:text-dark-6">
            Atualize as informações do tipo de quarto e seus preços por ocupação
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="mb-6 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-body-color dark:text-dark-6">
                  Configure preços específicos baseados na quantidade de hóspedes. Preços não incluídos serão removidos.
                </p>
              </div>
              <button
                type="button"
          ...
          <div className="flex gap-4">
            <Link
              href={`/room-types?hotelId=${formData.hotelId}`}
            ...
            >
              {submitting ? 'Salvando...' : 'Atualizar Tipo de Quarto'}
            </button>
          </div>
        </form>
      </div>
